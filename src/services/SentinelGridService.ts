      ...
      if (res.ok) {
        ...
        if (data && Array.isArray(data.cameras) && data.cameras.length > 0) {
          const items: SentinelCameraCatalogueItem[] = data.cameras.map((c: any) => ({
            ...
            bitrateKbps: 4096,
            rtspUrl: `rtsp://103.250.160.189:8554/stream/${c.id}`,
            whepUrl: `http://103.250.160.189:8889/stream/${c.id}/whep`,
...
corp8_pass = os.environ.get("CORP8_PASSWORD", "secret")
encoded_email = corp8_email.replace("@", "%40")

rtsp_url = f"rtsp://{encoded_email}:{corp8_pass}@103.250.160.189:8554/stream/${cam.id}"
...
gst-launch-1.0 rtspsrc location=rtsp://you%40example.com:PASS@103.250.160.189:8554/stream/${camId} protocols=tcp latency=200 \\
...
ffplay -rtsp_transport tcp rtsp://you%40example.com:PASS@103.250.160.189:8554/stream/${camId}

# Or HLS playback:
ffplay https://cctv.corp8.cloud/${camId}/index.m3u8

# FFprobe stream property & codec inspection:
ffprobe -rtsp_transport tcp rtsp://you%40example.com:PASS@103.250.160.189:8554/stream/${camId}`;
  }
...
enable=1
type=4
uri=rtsp://you%40example.com:PASS@103.250.160.189:8554/stream/${camId}
num-sources=1
gpu-id=0
...
`;
  ...
  public runPreSubmissionChecks(camId: string): SentinelComplianceCheckItem[] {
    const cam = this.getCameraById(camId) || {
      ...
      bitrateKbps: 4096,
      rtspUrl: `rtsp://103.250.160.189:8554/stream/${camId}`
    };
